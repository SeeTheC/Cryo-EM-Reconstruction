...
function [G_final,f_final,R_est,corr_error,G_init,f_init,R_init,clError,iteration] = solve3dObj(config)
    ...
    %% Apply L2 Norm Optimation using Alternate Minmization
        ...
        [G_final,R_est,trans_error_est,P_est,corr_error,iteration] = L2OptWithTransAlgo1(config.trans_projections,G_init,R_init,config.maxIteration,config.searchOffest,config);
        %[G_final,R_est,trans_error_est,P_est,corr_error,iteration] = L2OptWithTransAlgo1(projections,G_init,R_init,config.maxIteration,config.searchOffest,config);
        ...
        [f_final] = reconstructObjWarper(P_est,rots_final_aligned);
