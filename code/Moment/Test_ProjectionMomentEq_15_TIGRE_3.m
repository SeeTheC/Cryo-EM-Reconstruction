...
%% Taking projection : TIGRE

obj=obj1;

%[projections] = takeProjections(obj1,angles);

[projections] = takeProjectionWraper(obj,rots_true);
...
%% TESING For LEFT == RIGHT

offset=[0,pi/2,pi,3*pi/2,-2*pi,-pi/2,-pi,-3*pi/2];
n=numel(offset);
projIdx=1;
momentOrder=4;
fprintf('TESTING to find offset..\n');
ansProjLRError=[];     
for projIdx=1:size(projections,3)
    f=projections(:,:,projIdx);
    ax=angles(1,projIdx);ay=angles(2,projIdx);az=angles(3,projIdx);
    % Eq 16
    Pn_true=nthCentralMoment2D(f,momentOrder);
    AMtx=weightMtx(momentOrder,[ax,ay,az],[0,0,0]);
    Mn=nthCentralMoment3D(obj,momentOrder);
    Pn_est= AMtx*Mn;
    error=norm(Pn_true-Pn_est);
    fprintf('Proj:%d\t error:%f\tcorr:%f\n',projIdx, error,corr(Pn_true,Pn_est));
    ansProjLRError(projIdx)=error;
end
