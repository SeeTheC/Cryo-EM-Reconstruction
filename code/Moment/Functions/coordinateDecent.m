...
function [R_final,Im_final,An_final,error,iteration,ithError]=coordinateDecent(projMoment,R_init,searchArea,momentOrder,maxIteration)
     
    %% Init
     netError=0;    
     noOfProj=size(projMoment,3);
     [Pn] = assembleProjMoment(projMoment,momentOrder);
    ...
    for i=1:maxIteration
         ...
         parfor pIdx=1:1 %noOfProj
            ...
            bigAiCell=weightMtxUptoCMultAng(angSearch,momentOrder);   
            [sAn] = getSearchAn(An,bigAiCell,pIdx,AcH,momentOrder);            
            error=cellfun(@(x)( norm(Pn-(x*Im_est)) ),sAn,'UniformOutput',true);
            [v,idx]=min(error);
