...
for ii=list
    ...
    set(gca,'YDir','normal');
    if(size(alpha,1)==1)
        title(['Degree (',num2str(ii),'/',num2str(size(alpha,2)),'):',num2str(alpha(ii)*180/pi)]);
    else
        title(['Degree (',num2str(ii),'/',num2str(size(alpha,2)),'):','x:',num2str(alpha(1,ii)*180/pi),...
            ' y:',num2str(alpha(2,ii)*180/pi),...
            ' z:',num2str(alpha(3,ii)*180/pi)]);
    end
    pause(0.01);
    if savegif
